...
For speedtest the only relevant configuration is how often you want the check to happen. It is at 60 minutes by default which might be too much if you have limit on downloads. This is changed by editing `scrape_interval` under `speedtest` in [/prometheus/prometheus.yml](./prometheus/prometheus.yml).
...
username - admin
password - password (Password is stored in the `config.monitoring` env file)
...
To remove `internet-monitoring` from your system, run the following commands (assuming the default install location of `~`, your home directory):
...
Thanks to [@geerlingguy](https://github.com/geerlingguy) for making the [original project](https://github.com/geerlingguy/internet-monitoring) this fork is based on.

Thanks to [@vegasbrianc](https://github.com/vegasbrian) work on making a [super easy docker](https://github.com/vegasbrianc/github-monitoring) stack for running prometheus and grafana.
